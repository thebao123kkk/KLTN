...
def run_check(topo):
    ...
    vip_tests = [
        ('NT_ADM01', '10.20.10.1', 'NT ADMIN VLAN VIP'),
        ('NT_SAL01',  '10.20.20.1', 'NT SALES VLAN VIP'),
        ('HCM_ADM01', '10.30.10.1', 'HCM ADMIN VLAN VIP'),
        ('HCM_SAL01',  '10.30.20.1', 'HCM SALES VLAN VIP'),
    ]
    ...
    info('\n[TEST 2] VRRP Master state trên NT_DIST01...\n')
    ...
    if dist1:
        ...
        if '10.20.10.1' in vip_on_dist1:
            info(f'  ✓ NT_DIST01 đang giữ VIP 10.20.10.1 (MASTER)\n')
            results.append(True)
        else:
            error(f'  ✗ NT_DIST01 KHÔNG giữ VIP (keepalived có thể chưa start)\n')
            results.append(False)

    # ── Test 3: Fault test — DIST01 down ─────────────────
    info('\n[TEST 3] Fault test — NT_DIST01 DOWN → VIP chuyển DIST02...\n')
    src = net.get('NT_ADM01')
    dist1 = net.get('NT_DIST01')
    dist2 = net.get('NT_DIST02')

    if src and dist1 and dist2:
        vip = '10.20.10.1'

        # Baseline
        ok = _ping_once(net, 'NT_ADM01', vip)
        if not ok:
            error(f'  ✗ Baseline VIP ping FAIL — bỏ qua fault test\n')
        else:
            info(f'  ✓ Baseline VIP OK\n')

            # Tắt NT_DIST01 (kill keepalived + ip down)
            info(f'  → Kill keepalived trên NT_DIST01...\n')
            ...
            for _ in range(10):
                time.sleep(1)
                ok = _ping_once(net, 'NT_ADM01', vip, timeout=1)
                if ok:
            ...
            info('\n[TEST 4] Khôi phục NT_DIST01 → preempt MASTER...\n')
            for intf in dist1.intfList():
            ...
            vip_on_dist1 = dist1.cmd('ip addr show | grep "10.20.10.1"')
            if '10.20.10.1' in vip_on_dist1:
                info(f'  ✓ NT_DIST01 preempt thành công, VIP trở về DIST01\n')
                results.append(True)
            else:
                error(f'  ✗ NT_DIST01 chưa preempt (có thể cần thêm thời gian)\n')
                results.append(False)
    else:
